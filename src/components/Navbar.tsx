...
import { Menu, X, Phone, MapPin, Snowflake, Globe, ShoppingBag } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ christmasAdminEnabled = true, onOpenOrderModal }) => {
              ...
              <div className="flex gap-2 items-center">
                <div className="relative" ref={langMenuRef}>
                  <button
                    ...
                    className={`px-3 py-2 rounded-full border flex items-center justify-center gap-2 text-sm font-semibold transition-all h-10 ${scrolled ? 'border-bakery-200 text-bakery-700 bg-white hover:bg-bakery-50' : 'border-white/40 text-white bg-white/10 hover:bg-white/20 backdrop-blur'
                      } `}
                    ...
                    <div className={`absolute right-0 mt-2 w-44 rounded-xl shadow-xl border ${scrolled ? 'bg-white border-stone-100' : 'bg-white border-white/10'}`}>
                      {options.map((option) => (
                        <button
                          ...
                          className={`w-full px-4 py-2 text-left flex items-center gap-2 text-sm hover:bg-bakery-50 ${option.code === language ? 'font-bold text-bakery-700' : 'text-stone-600'
                            } `}
                          role="option"
                ...
                {christmasAdminEnabled && (
                  <button
                    ...
                    className={`p-2 rounded-full transition-all duration-300 hover:scale-110 active:scale-95 h-10 w-10 flex items-center justify-center ${scrolled
                      ? christmasEnabled
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-full font-bold transition-all transform hover:scale-105 h-10 ${scrolled
                    ? 'bg-bakery-500 text-white hover:bg-bakery-600 shadow-md'
                    : 'bg-white text-bakery-900 hover:bg-bakery-50 shadow-lg'
                    } `}
                >
                  <ShoppingBag size={18} />
                ...
                <a
                  ...
                  className={`px-5 py-2.5 rounded-full font-bold transition-all shadow-md flex items-center gap-2 ${scrolled
          ...
          <div className="lg:hidden bg-white border-t border-bakery-100 shadow-xl absolute w-full top-full left-0">
            <div className="px-4 py-4 space-y-3">
              ...
              {christmasAdminEnabled && (
                <button
                  onClick={toggleChristmasMode}
                  className={`w-full text-left px-4 py-3 rounded-lg text-lg font-medium border-l-4 transition-all flex items-center gap-3 ${christmasEnabled
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {options.map((option) => (
                  <button
                    ...
                    className={`flex-1 min-w-[30%] px-3 py-2 rounded-lg border text-sm font-semibold flex items-center justify-center gap-2 ${option.code === language ? 'border-bakery-500 text-bakery-700 bg-bakery-50' : 'border-stone-200 text-stone-600'
                      } `}
                  >
